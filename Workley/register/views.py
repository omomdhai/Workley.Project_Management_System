...
def tasks(request):
    ...
    task_history  = Task.objects.filter().values()
    task_history1  = Task.objects.filter().values('project','task_name','status','assign')
    return render(request, 'register/tasks.html',{'task':task_history1})
